...
var AssetCollector = function (type, collection, tags, brand, basedir) {
...
AssetCollector.prototype = {
	...
	collect: function () {
		...
		_.each(this.collection.locations, function (entry, key) {
			...
			var fullpath = path.join(this.basedir, key);
	...
	parseSection: function (assetConfig, assetPath, callback) {
		...
		fs.realpath(assetPath, (function (err, assetFullPath) {
				...
				switch (assetConfig.pattern) {
					...
					case 'bower':
						var dependencies = wiredep({
							directory: assetFullPath,
							exclude: modifier.exclude
						});

						callback(dependencies.js);
						break;
					...
					case 'file':
						...
						callback([assetFullPath]);
						break;

					case 'folder':
//						if( this.tags.length ) break; // Tags not supported at the moment
						this.getFolderAssets(assetFullPath, this.fileExtension, null, null, (function (files) {

							callback(this.filterAssets(files, assetFullPath, modifier, this.fileExtension));
						}).bind(this));
						break;

					case 'module_flat':
						this.getModuleAssets(assetFullPath, this.fileExtension, '', {}, (function (result) {

							callback(this.filterAssets(result, assetFullPath, modifier, this.fileExtension));
						}).bind(this));
						break;

					case 'module':
						this.getModuleAssets(assetFullPath, this.fileExtension, this.fileExtension, modifier, callback);
						break;

					case 'package':
						this.getPackageAssets(assetFullPath, this.fileExtension, this.fileExtension, modifier, callback);
						break;
...
//				var brandFiles;
	...
	isFileIncluded: function (fileName, modifier) {
		...
		if (_.isArray(modifier.include)) {
			...
			// check if any rule matches exactly
			if (modifier.include[fileName]) {
				include = modifier.include[fileName];
			}
			// check if any rule with * matches the file
			else {
				_.each(_.keys(modifier.include), function (value) {

					var basePath = value.replace(wildcard, '');

					if (wildcard.test(value)) {

						if (fileName.indexOf(basePath) == 0) {
							include = modifier.include[value];
						}
					}
				})
			}
		...
		if (_.isArray(modifier.exclude)) {
			...
			// check if any rule matches exactly
			if (modifier.exclude[fileName]) {
				exclude = modifier.exclude[fileName];
			}
			// check if any rule with * matches the file
			else {
				_.each(_.keys(modifier.exclude), function (value) {
					...
					if (wildcard.test(value)) {
						...
						if (fileName.indexOf(basePath) == 0) {
							exclude = modifier.exclude[value];
						}
					}
				})
			}
