...
AssetCollector.prototype = {
	...
	parseSection : function(assetConfig, assetPath, callback)
	{
		...
		fs.realpath(path.join(process.cwd(), assetPath), (function(err, assetPath){
			...
			{
//	console.log('parseSection', assetConfig.pattern)
				switch(assetConfig.pattern)
				{
					case 'file':
						...
						if( !this.hasValidFileEnd(assetPath, this.fileExtension) ) break;
						if( !this.isFileIncluded(assetPath, modifier) ) break;
...
//				if( this.tags.length ) break; // Tags not supported at the moment
					...
					case 'module_flat':
						files = this.getModuleAssets(assetPath, this.fileExtension);
						files = this.filterAssets(files, assetPath, modifier, this.fileExtension);
						callback(files);
						break;
